...
<body>
    ...
    <section id="content">
        ...
        <?php
$sentencia = $connect->prepare("SELECT sexd FROM doctor INNER JOIN users ON users.id = doctor.userid WHERE users.id = $id");
$sentencia->execute();
...
<h1 class="title"><?php if ($sexd == 'Femenino') { echo 'Bienvenida Dra. '; } else { echo 'Bienvenido Dr. '; } ?><?php echo '<strong>' . $_SESSION['name'] . '</strong>'; ?></h1>
                ...
                <div class="card">
                    <div class="head">
                        <div>
                            ...
                            <?php
                            $sql = "SELECT COUNT(*) total FROM patients INNER JOIN events ON patients.idpa = events.idpa INNER JOIN doctor ON events.idodc = doctor.idodc WHERE doctor.userid = $id";
                ...
                <div class="card">
                    <div class="head">
                        <div>
                            ...
                            <?php
                            $sql = "SELECT COUNT(*) total FROM events INNER JOIN doctor ON events.idodc = doctor.idodc WHERE doctor.userid = $id";
                            $result = $connect->query($sql); //$pdo sería el objeto conexión
                            $total = $result->fetchColumn();
