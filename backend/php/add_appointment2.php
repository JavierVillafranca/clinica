...
require_once('../../../backend/bd/Conexion.php'); 
 ...
 if(isset($_POST['add_appointment2']))
 {
    ...
    // Verificar si el periodo de tiempo está ocupado por otro doctor
    $query = "SELECT COUNT(*) FROM events WHERE idodc = :idodc AND ((start <= :start AND end >= :start) OR (start <= :end AND end >= :end))";
    ...
    if ($count > 0) {
        // Mostrar mensaje de error si el periodo está ocupado por otro doctor
